...
| `merge.sh` | bezpieczna integracja lokalnej gałęzi feature z wymaganym komunikatem merge; preflight worktree (także plików ignored) i lokalnego targetu, kontrola świeżości `origin/<target>`, blokada równoległych uruchomień, `--dry-run`, jawny tryb `--offline` i atomowy cleanup | `<repo>/scripts/merge.sh` |
...
Testy skryptów szablonowych żyją w `templates/tests/` (odpalane w CI, job „Templates”). Platform
używa kanonicznego `templates/merge.sh` przez cienki delegat [`../merge.sh`](../merge.sh), dzięki
czemu własna kopia helpera nie może się rozjechać z kanonem.
